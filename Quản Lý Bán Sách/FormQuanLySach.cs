...
namespace Quản_Lý_Bán_Sách
{
    public partial class FormQuanLySach : Form
    {
        ...
        string ChuoiKetNoi = @"Data Source=(local);Initial Catalog=QuanLyBanSach;Integrated Security=True";
        string Lenh;
        ...
        void HienDuLieu()
        {
            ...
            int i = 0;
            TongTien = 0;
            Lenh = @"SELECT       Sach.MaSach, Sach.Ten, Sach.SoLuong, Sach.DonGia, Sach.SoLuong*Sach.DonGia, NhaXuatBan.TenNXB
            ...
            while (DocDuLieu.Read())
            {
                ...
                //listView.Items[i].SubItems.Add((int.Parse(DocDuLieu[2].ToString())*int.Parse(DocDuLieu[3].ToString())).ToString());
        ...
        private void buttonLuu_Click(object sender, EventArgs e)
        {
            ...
            Lenh = @"SELECT       MaNXB
                     FROM            NhaXuatBan
                     WHERE        (TenNXB = N'"+comboBoxNXB.Text+"')";
            ThucHien = new SqlCommand(Lenh, KetNoi);
            KetNoi.Open();
            DocDuLieu = ThucHien.ExecuteReader();
            while (DocDuLieu.Read())
            ...
            Lenh = @"INSERT INTO Sach
                         (MaSach, Ten, SoLuong, DonGia, MaNXB)
                     VALUES       (N'"+textBoxMaSach.Text+@"', 
                                   N'"+textBoxTenSach.Text+@"', 
                                   "+textBoxSoLuong.Text+@", 
                                   "+textBoxDonGia.Text+@", 
                                   N'" + MaNXB + "')";
            KetNoi = new SqlConnection(ChuoiKetNoi);
        ...
        private void treeView_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            Lenh = @"SELECT       Sach.Ten
                     FROM         Sach INNER JOIN
                                  NhaXuatBan ON Sach.MaNXB = NhaXuatBan.MaNXB
                     WHERE        (NhaXuatBan.TenNXB = N'" + treeView.SelectedNode.Text + "')";
            ThucHien = new SqlCommand(Lenh, KetNoi);
            KetNoi.Open();
            DocDuLieu = ThucHien.ExecuteReader();
